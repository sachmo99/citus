...
static bool PlacementHasHealtyPlacementOnAnotherGroup(GroupShardPlacement
													  *sourcePlacement);
...
ErrorIfNodeContainsNonRemovablePlacements(WorkerNode *workerNode)
{
	...
	{
		if (!PlacementHasHealtyPlacementOnAnotherGroup(placement))
		{
			Oid relationId = RelationIdForShard(placement->shardId);
			char *qualifiedRelationName = generate_qualified_relation_name(relationId);

			ereport(ERROR, (errmsg("cannot remove or disable the node "
								   "%s:%d because there are some Citus tables "
								   "with the only healthy shard placements on "
								   "the node", workerNode->workerName,
...
/*
 * PlacementHasHealtyPlacementOnAnotherGroup returns true if there is at least
 * one more healthy placement of the input sourcePlacement on another group.
 */
static bool
PlacementHasHealtyPlacementOnAnotherGroup(GroupShardPlacement *sourcePlacement)
{
